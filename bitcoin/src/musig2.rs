...
use secp256k1::{musig::{new_musig_nonce_pair, MusigAggNonce, MusigKeyAggCache, MusigPubNonce, MusigSecNonce, MusigSession, MusigSessionId, MusigTweakErr}, Message, PublicKey, Scalar, Secp256k1, SecretKey, Signing, Verification};
...
pub fn create_musig_key_agg_cache<C: Verification>(secp: &Secp256k1<C>, pubkeys: &[PublicKey]) -> MusigKeyAggCache {

    MusigKeyAggCache::new(&secp, pubkeys)
}

/// Apply "x-only" tweaking to a public key in a [`secp256k1::musig::MusigKeyAggCache`].
pub fn tweak_taproot_key<C: Verification>(secp: &Secp256k1<C>, musig_key_agg_cache: &mut MusigKeyAggCache, tweak: &Scalar) -> Result<PublicKey, Error>
{
